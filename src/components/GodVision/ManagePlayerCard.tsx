...
import { getManagePlayerCard } from "../../services/getPageData";
...
const ManagePlayerCard: FC<Props> = ({ player, icon, role, type }) => {
  ...
  const { dataDictionary, language } = useSelector((state: AppState) => ({
    ...state.playersDataState,
    ...state.languageState,
  }));

  const { dead } = getManagePlayerCard(language);
  ...
  return (
    ...
    >
      <div className={styles.hedearContainer} style={{ color: color }}>
        ...
        <Link to={`/god-vision/${player}`} style={{ color: color }}>
          {role}{" "}
          {dataDictionary[player]?.alive ? (
            ""
          ) : (
            <span className={styles.deadStatus}>{dead}</span>
          )}
        </Link>
      </div>
